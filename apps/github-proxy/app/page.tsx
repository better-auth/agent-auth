	...
	return (
		<div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-inset">
			<div className="pointer-events-none absolute inset-0">
				<div className="absolute top-0 left-1/2 h-[700px] w-[700px] -translate-x-1/2 -translate-y-1/3 rounded-full bg-gh-green/4 blur-[160px]" />
			</div>
			...
			<main className="relative z-10 flex w-full max-w-sm flex-col items-center gap-10 px-6">
				<div className="flex flex-col items-center gap-5">
					...
					<div className="flex flex-col items-center gap-2 text-center">
						...
						<p className="max-w-xs text-muted text-sm leading-relaxed">
							Proxy GitHub API access for AI agents through the Agent Auth
							Protocol.
						</p>
					</div>
				</div>
				...
				<div className="w-full rounded-xl border border-border bg-surface p-6">
					<div className="flex flex-col gap-4">
						<button
							className="group flex h-10 w-full cursor-pointer items-center justify-center gap-2 rounded-md bg-gh-green font-medium text-gh-btn-text text-sm transition-colors hover:bg-gh-green-hover active:scale-[0.98] disabled:pointer-events-none disabled:opacity-60"
							disabled={signingIn}
							onClick={handleSignIn}
						>
							{signingIn ? <Spinner /> : "Sign in with GitHub"}
						</button>

						<p className="text-center text-muted text-xs">
							Grants access to your repositories, issues, pull requests, and
							workflows
						</p>
					</div>
				</div>
