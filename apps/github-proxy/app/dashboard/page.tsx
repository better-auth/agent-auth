...
function CopyButton({ value }: { value: string }) {
	...
	return (
		<button
			className="cursor-pointer rounded-md px-2 py-0.5 text-accent text-xs transition-colors hover:bg-accent/10"
			onClick={copy}
		>
			{copied ? "Copied!" : "Copy"}
		</button>
...
export default function DashboardOverview() {
	...
	const statCards = [
		{ label: "Total Agents", value: stats?.totalAgents ?? "—" },
		{
			...
			color: "text-gh-green-emphasis",
		},
		{
			label: "Pending",
			value: stats?.pendingAgents ?? "—",
			color: "text-gh-attention",
		},
		{ label: "Hosts", value: stats?.totalHosts ?? "—", color: "text-gh-done" },
	...
	return (
		<div className="mx-auto w-full max-w-3xl px-6 py-8">
			<div className="flex flex-col gap-8">
				<div>
					<h1 className="font-semibold text-foreground text-lg">Overview</h1>
					<p className="mt-1 text-muted text-sm">
						Your GitHub account is connected. AI agents can access repositories,
						issues, and pull requests through Agent Auth.
					</p>
				</div>
				...
				<div className="grid grid-cols-5 gap-3">
					{statCards.map((s) => (
						...
						>
							...
							<p
								className={`mt-1 font-semibold text-xl ${s.color ?? "text-foreground"}`}
							>
								{s.value}
							</p>
						</div>
					...
					<div className="rounded-md border border-border bg-surface">
						...
						<div className="flex items-center justify-between px-4 py-3">
							...
							<div className="flex items-center gap-1">
								<code className="rounded-md bg-inset px-2 py-0.5 font-mono text-foreground text-xs">
									{session?.user.id}
								</code>
					...
					<div className="rounded-md border border-border bg-surface">
						{endpoints.map((ep, i) => (
							...
							>
								...
								<div className="flex items-center gap-1">
									<code className="rounded-md bg-inset px-2 py-0.5 font-mono text-foreground/70 text-xs">
										{ep.path}
									</code>
