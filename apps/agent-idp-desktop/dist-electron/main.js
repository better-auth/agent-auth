...
var lastSeenIds = [];
var storePath = import_node_path.default.join(
	import_electron.app.getPath("userData"),
	"agent-auth-store.json",
);
function loadStore() {
	try {
		if (import_node_fs.default.existsSync(storePath)) {
			store = JSON.parse(
				import_node_fs.default.readFileSync(storePath, "utf-8"),
			);
		}
	} catch {
		store = {};
...
function createTrayIcon() {
	...
	const resourcesPath = import_electron.app.isPackaged
		? import_node_path.default.join(process.resourcesPath, "resources")
		: import_node_path.default.join(__dirname, "../resources");
...
function createMainWindow() {
	...
	if (process.env.VITE_DEV_SERVER_URL) {
		...
		mainWindow.loadFile(
			import_node_path.default.join(__dirname, "../dist/index.html"),
		);
	}
	mainWindow.on("blur", () => {
...
function registerIpcHandlers() {
	...
	import_electron.ipcMain.handle("api:fetch", (_event, apiPath, options) => {
