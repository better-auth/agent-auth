...
async function getSessionAndProvider(request: Request, encodedName: string) {
  ...
  if (!session) {
    return {
      error: Response.json({ error: "Sign in to manage your providers" }, { status: 401 }),
    };
  }
  ...
  const [row] = await db
    .select()
    .from(provider)
    .where(and(eq(provider.name, name), eq(provider.submittedBy, session.user.id)))
    .limit(1);

  if (!row) {
    return {
      error: Response.json({ error: "Provider not found or you don't own it" }, { status: 404 }),
    };
  }
...
export async function PATCH(request: Request, { params }: { params: Promise<{ name: string }> }) {
  try {
    ...
    await db.update(provider).set(updates).where(eq(provider.name, result.name));
