  ...
  return (
    <div className="min-h-dvh flex flex-col">
      ...
      <div className="flex-1 px-4 sm:px-6 lg:px-8 py-8 max-w-3xl mx-auto w-full">
        ...
        {isOwner ? (
          <TogglePublicButton providerName={row.name} initialPublic={row.public} />
        ) : (
          !row.public && (
        ...
        <div className="space-y-4 mb-10">
          ...
          <p className="text-sm text-foreground/55 leading-relaxed">{row.description}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
          <InfoCard icon={<Globe className="h-3.5 w-3.5" />} label="Issuer" value={row.issuer} />
          <InfoCard
          ...
          <InfoCard
            ...
            label="Last Verified"
            value={row.lastCheckedAt ? new Date(row.lastCheckedAt).toLocaleDateString() : "Never"}
          />
        </div>
