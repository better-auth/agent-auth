...
import { ArrowUpRight, Globe, Loader2, Pencil, Trash2, X } from "lucide-react";
...
export function MyProvidersList({ initialProviders }: { initialProviders: MyProvider[] }) {
  ...
  const togglePublic = async (p: MyProvider) => {
    ...
    try {
      const res = await fetch(`/api/my-providers/${encodeURIComponent(p.name)}`, {
        method: "PATCH",
      ...
      if (res.ok) {
        setProviders((prev) =>
          prev.map((pr) => (pr.name === p.name ? { ...pr, public: !pr.public } : pr)),
        );
        router.refresh();
      }
    } finally {
  ...
  return (
    <div className="space-y-3">
      {providers.map((p) => (
        <div key={p.id} className="border border-foreground/8 bg-foreground/[0.015]">
          {editing === p.name ? (
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-2">
                ...
                >
                  {actionLoading === p.name ? <Loader2 className="h-3 w-3 animate-spin" /> : null}
                  Save
                </button>
                <button
                  onClick={() => setEditing(null)}
            ...
            <div className="p-5 space-y-4">
              <p className="text-sm text-foreground/70">
                Delete <span className="font-medium text-foreground">{p.displayName}</span>? This
                cannot be undone.
              </p>
              <div className="flex gap-2">
                <button
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      <h3 className="text-sm font-medium text-foreground">{p.displayName}</h3>
                      {p.verified && (
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    >
                      {actionLoading === p.name && <Loader2 className="h-3 w-3 animate-spin" />}
