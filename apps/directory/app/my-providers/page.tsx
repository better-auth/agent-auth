...
export default async function MyProvidersPage() {
  ...
  const rows = await db.select().from(provider).where(eq(provider.submittedBy, session.user.id));
    ...
    <div className="min-h-dvh flex flex-col">
      ...
      <div className="flex-1 px-4 sm:px-6 lg:px-8 py-8 max-w-4xl mx-auto w-full">
        <div className="space-y-2 mb-8">
          <h1 className="text-lg font-semibold text-foreground">My Providers</h1>
          <p className="text-xs text-foreground/45 leading-relaxed">
            Manage providers you've submitted to the directory.
          </p>
        </div>

        {providers.length === 0 ? (
          <div className="text-center py-16 space-y-3">
            <p className="text-sm text-foreground/40">You haven't submitted any providers yet.</p>
            <Link
              href="/submit"
