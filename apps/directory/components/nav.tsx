      ...
      {open && (
        <div className="fixed inset-0 top-[calc(var(--nav-h,45px))] z-50 bg-background border-t border-foreground/[0.06]">
          <div className="flex flex-col p-5 space-y-1">
            ...
            >
              <span>Theme</span>
              <span className="text-xs text-foreground/30 capitalize">{resolvedTheme}</span>
            </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 z-50 min-w-[200px] border border-foreground/[0.08] bg-background shadow-lg shadow-black/[0.08]">
          <div className="px-3.5 py-3 border-b border-foreground/[0.06]">
            {user.name && (
              <p className="text-xs font-medium text-foreground/80 truncate">{user.name}</p>
            )}
            {user.email && (
          ...
          <div className="border-t border-foreground/[0.06] py-1">
            ...
            >
              ...
              <span className="text-[10px] text-foreground/30 capitalize">{resolvedTheme}</span>
            </button>
          </div>
...
export function Nav() {
  ...
  return (
    <nav
      className="shrink-0 flex items-center border-b border-foreground/[0.06]"
      style={{ "--nav-h": "45px" } as React.CSSProperties}
    >
      ...
      <div className="ml-auto flex items-center gap-2 px-4 sm:px-6">
        <div className="hidden sm:flex items-center gap-2">
          ...
          {!session && (
            <>
              <span className="text-foreground/15 text-[10px] select-none ml-2">|</span>
              <ThemeToggle />
            </>
          )}
          <span className="text-foreground/15 text-[10px] select-none ml-2">|</span>
