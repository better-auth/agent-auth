...
export function getClientForUser(userId: string): AgentAuthClient {
  ...
  client = new AgentAuthClient({
    storage,
    directoryUrl: process.env.AGENT_AUTH_DIRECTORY_URL ?? "https://agent-auth.directory",
    allowDirectDiscovery: true,
    hostName: process.env.AGENT_AUTH_HOST_NAME ?? "Agent Auth Directory",
 ...
 */
const ADMIN_TOOLS = ["rotate_host_key", "rotate_agent_key"];
...
export function getToolsForUser(userId: string): AgentAuthTool[] {
  ...
  return tools.map((tool) => {
    ...
    return {
      ...tool,
      async execute(args: Record<string, unknown>, context?: { signal?: AbortSignal }) {
        try {
          ...
          if (isTimeout && pending) {
            ...
            const agentId =
              err && typeof err === "object" && "agentId" in err
...
export function jsonSchemaToZod(params: ToolParameters, zod: typeof z): ZodRawShape | undefined {
