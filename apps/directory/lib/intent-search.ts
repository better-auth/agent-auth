...
function loadPipeline(): Promise<Pipeline | null> {
  ...
  pipelineState.loading = (async () => {
    try {
      ...
      console.warn("Failed to load embedding model, using keyword fallback:", err);
...
function keywordFallback<T extends ProviderForSearch>(providers: T[], intent: string): T[] {
