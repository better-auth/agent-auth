...
function EventTypeBadge({ type }: { type: string }) {
	...
	return (
		<span
			className={`inline-flex items-center rounded-full px-2 py-0.5 font-medium text-[11px] ${styles[category] ?? "bg-zinc-500/10 text-zinc-400"}`}
		>
			{type}
		</span>
	);
			...
			<div className="flex flex-col gap-6">
				<div className="flex items-start justify-between">
					...
					<div className="flex items-center gap-3">
						...
						>
							<span
								className={`h-1.5 w-1.5 rounded-full ${autoRefresh ? "animate-pulse bg-emerald-400" : "bg-muted/50"}`}
							/>
