...
export const db = new Database("cloudflare-proxy.db");
...
export function getRegistration(key: string): string | undefined {
	const row = db
		.prepare("SELECT value FROM registration WHERE key = ?")
