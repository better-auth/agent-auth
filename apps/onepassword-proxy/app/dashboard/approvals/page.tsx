	...
	return (
		<div className="mx-auto w-full max-w-3xl px-6 py-8">
			<div className="flex flex-col gap-6">
				<div>
					<h1 className="text-lg font-semibold text-white">Approval Requests</h1>
				...
				{loading ? (
					...
					<div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-16">
						...
						<p className="mt-1 text-xs text-muted/60">CIBA approval requests will appear here automatically.</p>
					</div>
				) : (
					<div className="flex flex-col gap-3">
						{requests.map((req) => (
							<div key={req.approval_id} className="rounded-lg border border-border bg-surface">
									...
									<div className="flex items-start justify-between gap-4">
										<div className="flex-1 min-w-0">
											<div className="flex items-center gap-2">
												...
												<span className="inline-flex items-center rounded-full border border-op-attention/40 bg-op-attention/15 px-2 py-0.5 text-[11px] font-medium text-op-attention">
													pending
												</span>
											</div>
											{req.binding_message && (
										...
										<div className="flex gap-2 shrink-0">
											<button
												...
												className="cursor-pointer rounded-md bg-op-green px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-op-green-hover disabled:opacity-50"
											>
											...
											<button
												...
												disabled={acting === req.approval_id}
												className="cursor-pointer rounded-md border border-border bg-surface px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:border-op-danger/50 hover:text-op-danger disabled:opacity-50"
											>
												Deny
											</button>
										</div>
