...
function EventTypeBadge({ type }: { type: string }) {
	return (
		<span
			className="inline-flex items-center rounded-full border border-border bg-surface px-2 py-0.5 text-[11px] font-medium text-foreground/70"
		>
			{type}
		</span>
	);
			...
			<div className="flex flex-col gap-6">
					...
					<div className="flex items-center gap-3">
						<button
							...
							className={`cursor-pointer flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-xs transition-colors ${
								autoRefresh
									? "border-op-green-emphasis/40 text-op-green-emphasis"
									: "border-border text-muted hover:text-foreground"
							}`}
						>
							<span className={`h-1.5 w-1.5 rounded-full ${autoRefresh ? "bg-op-green-emphasis animate-pulse" : "bg-muted/50"}`} />
				...
				{loading ? (
					...
					<div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-16">
						...
						<p className="mt-1 text-xs text-muted/60">Events will appear here as agents interact with the system.</p>
					</div>
				) : (
					<>
											...
											<div className="flex-1 min-w-0 flex items-center gap-3">
												{log.agentId && (
													<span className="text-xs text-muted truncate">
														agent: <code className="text-foreground/70">{log.agentId.slice(0, 8)}…</code>
													</span>
												)}
												{log.hostId && (
													<span className="text-xs text-muted truncate">
														host: <code className="text-foreground/70">{log.hostId.slice(0, 8)}…</code>
													</span>
												)}
												{log.actorId && (
													<span className="text-xs text-muted truncate">
														by <code className="text-foreground/70">{log.actorId.slice(0, 8)}…</code>
													</span>
												)}
											</div>
											<span className="text-[11px] text-muted/60 shrink-0">{formatTimestamp(log.createdAt)}</span>
										</div>
										{isExpanded && log.data && (
