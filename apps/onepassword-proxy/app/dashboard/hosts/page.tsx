...
function StatusBadge({ status }: { status: string }) {
	const styles: Record<string, string> = {
		...
		pending_enrollment: "border-op-attention/40 bg-op-attention/15 text-op-attention",
...
export default function HostsPage() {
	...
	const filters = ["all", "active", "pending", "revoked"];
		...
		<div className="mx-auto w-full max-w-3xl px-6 py-8">
			<div className="flex flex-col gap-6">
				<div className="flex items-start justify-between">
					<div>
						<h1 className="text-lg font-semibold text-white">Hosts</h1>
						<p className="mt-1 text-sm text-muted">
							Registered hosts (agent platforms) and their default capabilities.
						</p>
					</div>
				...
				{loading ? (
					...
					<div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border py-16">
						...
						<p className="mt-1 text-xs text-muted/60">Hosts will appear here once they register.</p>
					</div>
				) : (
					<div className="flex flex-col gap-2">
						{hosts.map((host) => {
							...
							return (
								<div key={host.id} className="rounded-lg border border-border bg-surface">
									<button
									...
									>
										<div className="flex-1 min-w-0">
											<div className="flex items-center gap-2">
												<span className="text-sm font-medium text-white truncate">{host.name || host.id.slice(0, 12)}</span>
												<StatusBadge status={host.status} />
											</div>
											<p className="mt-0.5 text-xs text-muted">
												{host.default_capabilities?.length ?? 0} default capabilities · {timeAgo(host.created_at)}
											</p>
										</div>
										<svg
										...
										<div className="border-t border-border px-4 py-4">
											<div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-4">
												...
												<div>
													...
													<p className="text-xs text-foreground">{timeAgo(host.activated_at)}</p>
												</div>
												<div>
											...
											{host.default_capabilities && host.default_capabilities.length > 0 && (
												<div className="mb-4">
													<p className="mb-2 text-[10px] uppercase tracking-widest text-muted">Default Capabilities</p>
													<div className="flex flex-wrap gap-1.5">
														{host.default_capabilities.map((cap) => (
															<code key={cap} className="rounded bg-background px-2 py-1 text-xs font-mono text-foreground">{cap}</code>
														))}
													</div>
												</div>
											)}
											...
											{host.status === "active" && (
												<button
													onClick={(e) => { e.stopPropagation(); handleRevoke(host.id); }}
