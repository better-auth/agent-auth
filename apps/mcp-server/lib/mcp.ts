...
// Short timeout so serverless functions don't block while polling.
// The approval URL is captured via onApprovalRequired and returned
// immediately to the client (ChatGPT) as a tool result.
...
export function getClientForUser(userId: string): AgentAuthClient {
  ...
  const storage = createUserStorage(pool, userId);

  client = new AgentAuthClient({
    storage,
    registryUrl: process.env.AGENT_AUTH_REGISTRY_URL,
    allowDirectDiscovery: true,
    hostName: process.env.AGENT_AUTH_HOST_NAME ?? "Agent Auth MCP Server",
 ...
 *
 * When approval is needed, the SDK will timeout quickly (500ms)
 * and we catch the error to return the approval URL instead
      ...
      async execute(
        ...
        try {
          ...
          if (isTimeout && pending) {
            ...
            // MCP URL-mode elicitation (SEP-1036 / spec 2025-11-25)
            // Clients that support it (e.g. ChatGPT) will show a
            // native "Open URL" button instead of a raw link.
...
// Reused from packages/cli/src/mcp.ts
