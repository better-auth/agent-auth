...
/**
 * Fetch the Agent Auth discovery document from a service URL.
 * Tries `/.well-known/agent-configuration` first, then known path prefixes.
 */
export async function discoverProvider(
	...
	for (const prefix of KNOWN_PATHS) {
		try {
			const res = await fetchWithTimeout(
				`${normalized}${prefix}/agent/discover`,
			);
			if (res.ok) {
				return (await res.json()) as ProviderConfig;
			}
