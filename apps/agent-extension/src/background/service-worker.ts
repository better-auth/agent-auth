...
const DIRECTORY_URL = __DIRECTORY_URL__;
...
async function updateAccount(accountId: string, patch: Partial<StoredAuthAccount>): Promise<void> {
  const accounts = await getAccounts();
  await saveAccounts(
  ...
  if (changeInfo.cause === "explicit" || changeInfo.cause === "expired_overwrite") {
    getAccounts().then((accounts) => {
      const affected = accounts.filter((account) => {
        try {
          return new URL(account.idpUrl).hostname === changeInfo.cookie.domain.replace(/^\./, "");
        } catch {
          return false;
        }
      });
...
async function refreshSession(accountId?: string): Promise<void> {
  ...
  if (account.sessionToken) {
    const accounts = (await getAccounts()).filter((candidate) => candidate.id !== account.id);
...
async function fetchDirectoryProviders(): Promise<DirectoryProvider[]> {
  const allProviders: DirectoryProvider[] = [];
  let page = 1;
  const limit = 100;

  while (true) {
    const res = await fetch(`${DIRECTORY_URL}/api/providers?page=${page}&limit=${limit}`, {
      method: "GET",
      headers: { accept: "application/json" },
      signal: AbortSignal.timeout(10_000),
    });
    if (!res.ok) break;
    const body = (await res.json()) as { providers?: DirectoryProvider[] };
...
async function trySessionForUrl(
  ...
  try {
    ...
    for (const cookie of cookies) {
      if (!cookie.value || cookie.value.length < 16) continue;
      try {
        const res = await fetch(`${providerUrl}/api/auth/get-session`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${cookie.value}`,
...
async function discoverDirectoryAccounts(force = false): Promise<{ discovered: number }> {
  ...
  try {
    providers = await fetchDirectoryProviders();
  } catch {
    return { discovered: 0 };
  }

  const accounts = await getAccounts();
  const knownUrls = new Set(accounts.map((a) => a.idpUrl.replace(/\/+$/, "")));
...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === ALARM_NAME) {
    discoverDirectoryAccounts().then(() => pollApprovals());
  }
});
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message.type === "discover-accounts") {
    discoverDirectoryAccounts(true).then(sendResponse);
    return true;
  }
});
