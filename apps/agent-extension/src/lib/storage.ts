...
export const storage = {
  ...
  setPrimaryAccountId: (accountId: string) => set({ primaryAccountId: accountId }),

  async upsertAccount(account: Omit<StoredAuthAccount, "id"> & { id?: string }) {
    const normalizedId = account.id ?? this.buildAccountId(account.idpUrl, account.user.id);
    const accounts = await this.getAccounts();
  ...
  async getLastSeenIds(accountId?: string): Promise<string[]> {
    if (accountId) {
      const account = (await this.getAccounts()).find((a) => a.id === accountId);
  ...
  async clearSession() {
    await remove(["accounts", "primaryAccountId", "idpUrl", "sessionToken", "user", "lastSeenIds"]);
  },
