...
export async function approveRequest(
	...
	try {
		...
		const res = await fetch(
			`${account.idpUrl}/api/auth/agent/approve-capability`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
...
export async function fetchReAuthConfig(): Promise<{
	...
	try {
		...
		return {
			allowedMethods:
				data.allowedMethods?.length > 0 ? data.allowedMethods : ["password"],
		};
	} catch {
...
export async function listHosts(): Promise<{
	...
	try {
		...
		if (!res.ok) {
			...
			return {
				error:
					(json as { message?: string }).message ?? "Failed to list devices",
			};
		}
		const json = await res.json();
...
export async function createHost(
	...
	try {
		...
		if (!res.ok)
			return {
				error:
					(json as { message?: string }).message ?? "Failed to create device",
			};
...
export async function revokeHost(hostId: string): Promise<{ error?: string }> {
	try {
		...
		if (!res.ok) {
			...
			return {
				error:
					(json as { message?: string }).message ?? "Failed to revoke device",
			};
		}
		return {};
	} catch {
...
export async function claimHost(hostId: string): Promise<{
	...
	try {
		...
		if (!res.ok)
			return {
				error:
					(json as { message?: string }).message ?? "Failed to link device",
			};
		return {
