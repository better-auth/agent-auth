...
export function App() {
  ...
  const handleSignedIn = useCallback((u: User) => {
    ...
    void storage.getAccounts().then((accounts) => setAccountCount(accounts.length));
  }, []);
  ...
  if (showAddAccount) {
    return <SignIn onSuccess={handleSignedIn} onCancel={() => setShowAddAccount(false)} />;
  }
      ...
      <nav className="flex border-b border-border">
        {TABS.map((t) => (
          ...
          >
            ...
            {tab === t.id && <span className="absolute bottom-0 inset-x-0 h-[2px] bg-foreground" />}
          </button>
        ))}
      </nav>
      ...
      {user && (
        <footer className="flex items-center justify-between px-4 py-2 border-t border-border bg-muted/20">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-muted-foreground/70">Connected</span>
          </div>
        </footer>
      )}
    </div>
