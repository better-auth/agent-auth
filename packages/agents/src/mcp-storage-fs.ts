...
async function encryptKeypair(
	...
	const { bytesToHex, utf8ToBytes } = await import("@noble/ciphers/utils.js");
	...
	const cipher = xchacha20poly1305(new Uint8Array(keyBytes), nonce);
	const encrypted = cipher.encrypt(data);
	const combined = new Uint8Array(nonce.length + encrypted.length);
	combined.set(nonce);
	combined.set(encrypted, nonce.length);
	return ENCRYPTED_PREFIX + bytesToHex(combined);
}
...
async function decryptKeypair(
	...
	const { xchacha20poly1305 } = await import("@noble/ciphers/chacha.js");
	const { hexToBytes } = await import("@noble/ciphers/utils.js");
	const { createHash } = await import("@better-auth/utils/hash");
...
export function createFileStorage(
	...
	function providerFile(name: string): string {
		...
		return path.join(providersDir, `${safe}.json`);
	}
	...
	return {
		...
		async saveHostKeypair(appUrl, data) {
			...
			writeJSON(hostsFile, hosts, true);
		},
		...
		async saveProviderConfig(name, config) {
			writeJSON(providerFile(name), config);
		},

		async getProviderConfig(name) {
			return readJSON<ProviderConfig>(providerFile(name));
		},

		async listProviderConfigs() {
			ensureDir(providersDir);
			const result: Array<{ name: string; config: ProviderConfig }> = [];
			...
			for (const file of files) {
				...
				const config = readJSON<ProviderConfig>(path.join(providersDir, file));
				if (config) {
