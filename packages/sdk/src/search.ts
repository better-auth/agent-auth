...
export interface ScoredMatch {
  cap: Capability;
  /** Relevance score: nameHits * 2 + descHits */
  score: number;
  /** Fraction of query terms that matched (0–1). */
  coverage: number;
}

/**
 * Search capabilities by query with relevance scores.
 * Supports glob (`*`, `?`), regex (`/pattern/flags`), and multi-term text search.
 */
export function matchQueryScored(query: string, capabilities: Capability[]): ScoredMatch[] {
  if (capabilities.length === 0) return [];

  const raw = query.trim();
  if (!raw) return capabilities.map((cap) => ({ cap, score: 1, coverage: 1 }));

  const regex = parseRegex(raw);
  if (regex)
    return matchPattern(regex, capabilities).map((cap) => ({
      cap,
      score: 1,
      coverage: 1,
    }));
  ...
  if (GLOB_CHARS.test(raw))
    return matchPattern(globToRegex(raw), capabilities).map((cap) => ({
      cap,
      score: 1,
      coverage: 1,
    }));
  ...
  if (terms.length === 0) return capabilities.map((cap) => ({ cap, score: 1, coverage: 1 }));
  ...
  for (const cap of capabilities) {
    ...
    for (const term of terms) {
      const inName =
        termMatchesText(term, nameLower) || nameTokens.some((t) => termMatchesToken(term, t));
      const inDesc =
        termMatchesText(term, descLower) || descTokens.some((t) => termMatchesToken(term, t));
    ...
    if (score > 0) {
      const coverage = (nameHits + descHits) / terms.length;
      scored.push({ cap, score, coverage });
    }
  }
 ...
 */
export function matchQuery(query: string, capabilities: Capability[]): Capability[] {
  return matchQueryScored(query, capabilities).map((s) => s.cap);
}
