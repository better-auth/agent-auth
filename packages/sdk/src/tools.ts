...
export function getAgentAuthTools(client: AgentAuthClient): AgentAuthTool[] {
  return [
...

    {
      name: "search",
      description:
        "Search for capabilities across all providers. This ALREADY searches both the local cache AND the directory in a single call — you do NOT need to call search_providers or list_providers afterwards. Results include provider info and supported modes so you can go straight to connect_agent. If a provider supports both 'delegated' and 'autonomous' modes, ask the user which they prefer BEFORE calling connect_agent. This is the ONLY search tool you need for finding capabilities.",
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description: "What you want to do (e.g. 'send email', 'deploy app', 'list repos')",
          },
          limit: {
            type: "number",
      ...
      async execute(args) {
        ...
        const hasConstrainable = results.some(
          (r) =>
            ...
            Object.keys(r.constrainable_fields as Record<string, unknown>).length > 0,
        );
        const hasDualMode = results.some(
          (r) => r.modes && r.modes.includes("delegated") && r.modes.includes("autonomous"),
        );
        return {
          results,
          ...(hasConstrainable && {
          ...
          ...(hasDualMode && {
            mode_selection_hint:
              "Some providers support both user-linked and independent access. BEFORE calling connect_agent, ask the user which they prefer. NEVER use the words 'delegated' or 'autonomous' when talking to the user — those are internal protocol terms. Instead present it naturally, e.g.: 'Would you like to connect your account so I act on your behalf, or should I work independently?' Pass mode='delegated' for user-linked or mode='autonomous' for independent.",
          }),
        };
      },
    },
...

    {
      ...
      description:
        "Search the directory for providers by name or intent. ALMOST NEVER NEEDED — the search tool already queries the directory. Only use this if search returned zero results AND you believe there may be a provider not yet indexed for the capability you want.",
      annotations: { readOnlyHint: true },
      parameters: {
        type: "object",
        properties: {
...

    {
      ...
      description:
        "Look up a provider by URL. Only needed if you have a specific URL that isn't in the directory.",
      annotations: { readOnlyHint: true },
      parameters: {
        type: "object",
        properties: {
...

    {
      ...
      parameters: {
        ...
        properties: {
          ...
          query: {
            type: "string",
            description: "Search query to filter capabilities by name or description",
          },
          agent_id: {
            type: "string",
            description: "Agent ID to see grant status (only after connect_agent)",
          },
          limit: {
            type: "number",
...

    {
      name: "connect_agent",
      description:
        "Connect to a provider and get an agent_id. Call ONCE, then use execute_capability for everything. Typical flow: search → ask user to choose mode (if provider supports both) → connect_agent with mode → execute_capability. NEVER re-call connect_agent for the same provider unless execute_capability returns 'agent_not_found' or 'revoked'. IMPORTANT: When capabilities have constrainable_fields, you MUST apply constraints to limit your scope (principle of least privilege). For example, if sending email and you know the recipient, constrain the 'to' field. IMPORTANT: If search results show a provider supports both modes, ask the user which they prefer BEFORE calling this tool — but NEVER use the words 'delegated' or 'autonomous' with the user. Present it naturally: 'connect your account' (delegated) vs 'let me work independently' (autonomous). Always pass the 'mode' parameter when both modes are available. If you omit 'mode', this tool will return a mode selection prompt as a fallback — but prefer asking upfront to avoid an extra round-trip.",
      ...
      parameters: {
        ...
        properties: {
          ...
          mode: {
            ...
            description:
              "Connection mode. 'delegated' = user connects their account (you act on their behalf). 'autonomous' = you work independently without a user account. NEVER expose these terms to the user — say 'connect your account' or 'work independently' instead. If omitted, the tool will return a prompt for the user to choose.",
          },
          name: {
            type: "string",
            description:
          ...
          force_approval: {
            type: "boolean",
            description:
              "Force the approval flow even if the agent would otherwise be auto-approved. Use this to switch the linked user account — the existing binding is reset during approval so a different user can authenticate.",
          },
        },
        required: ["provider"],
      },
      async execute(args, ctx) {
        ...
        const forceApproval = args.force_approval as boolean | undefined;

        if (!mode) {
          let supportedModes: string[] = ["delegated", "autonomous"];
          try {
            const config = await client.getProviderConfig(args.provider as string);
          ...
          const options = supportedModes.map((m) =>
            m === "autonomous"
              ? {
                  ...
                  description:
                    "I'll work on my own without needing your account. You can claim ownership of anything I create later.",
                }
              : {
                  mode: "delegated",
                  label: "Connect your account",
                  description:
                    "Link your account so I act on your behalf. You'll approve what I can do.",
                },
          );
          ...
          return {
            ...
            message:
              "Ask the user how they'd like to connect. NEVER show the raw mode names ('delegated'/'autonomous') — use the labels and descriptions below, or rephrase naturally.",
            options,
            instruction:
              "Present these options naturally to the user (e.g. 'Would you like to connect your account, or should I work independently?'). Then call connect_agent again with the chosen mode.",
          };
        }
          ...
          requests: {
            ...
            items: {
              ...
              properties: {
                ...
                arguments: {
                  type: "object",
                  description: "Arguments for the capability, conforming to its input schema",
                },
              },
              required: ["capability"],
            },
            description: "Array of capability executions to run in parallel. Max 50.",
          },
        },
...

    {
      name: "enroll_host",
      description:
        "Enroll host using a one-time enrollment token. Only needed when the host was pre-registered without a public key.",
      annotations: { readOnlyHint: true },
      parameters: {
        type: "object",
        properties: {
...

    {
      name: "claim_agent",
      description:
        "Claim an autonomous agent and take ownership of its resources. Use when a user wants to claim a site or project created by an autonomous AI agent. The user must approve via a browser-based flow. After claiming, the agent's resources (e.g. deployed sites) transfer to the user and you get an active agent connection.",
      annotations: { readOnlyHint: true, title: "Claim Agent" },
      parameters: {
        type: "object",
        properties: {
          provider: {
          ...
          agent_id: {
            type: "string",
            description: "ID of the autonomous agent to claim (shown on the site's claim banner)",
          },
          preferred_method: {
            type: "string",
            description: "Preferred approval method (e.g. device_authorization)",
          },
          login_hint: {
            type: "string",
            description: "Login hint for CIBA approval (e.g. user email)",
 ...
 * ```
 */
export function filterTools(tools: AgentAuthTool[], opts: FilterToolsOptions): AgentAuthTool[] {
...
export function toOpenAITools(tools: AgentAuthTool[], opts?: OpenAIToolsOptions): OpenAITools {
  ...
  for (const tool of tools) {
    ...
    const params = strict ? addAdditionalPropertiesFalse(tool.parameters) : tool.parameters;
