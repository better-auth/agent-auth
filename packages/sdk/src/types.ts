...
export interface CapabilitySearchResult extends Capability {
  ...
  provider_name: string;
  /** Connection modes the provider supports (e.g. ["delegated", "autonomous"]). */
  modes?: AgentMode[];
}
...
export interface AgentAuthClientOptions {
  ...
  storage?: Storage;
  /**
   * Directory URL for `searchProviders`.
   ...
   *
   * When `false` and a `directoryUrl` is set, `discoverProvider` will
   * only resolve providers through the directory — never fetch from
