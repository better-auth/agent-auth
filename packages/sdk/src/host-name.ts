...
export function detectTool(): ToolDetection | null {
  ...
  if (env.WINDSURF_SESSION_ID || env.WINDSURF_EXTENSION_ID || env.CODEIUM_WINDSURF) {
 ...
 *
 * Never returns null — falls back to a platform-based label
 * so that dynamically registered hosts always have a readable name.
 */
export function detectHostName(): string {
  return getDeviceName() ?? getPlatformLabel();
}

function getPlatformLabel(): string {
  try {
    const { platform, arch } = require("node:os") as typeof import("node:os");
    const os = platform();
