	...
	return createAuthEndpoint(
		...
		async (ctx) => {
			...
			const pendingCapIds = existingGrants
				.filter(
					(g) =>
						g.status === "pending" && (!g.grantedBy || g.grantedBy === ownerId)
				)
				.map((g) => g.capability);

			const alreadyActive = new Set(activeCapIds);
			const alreadyPending = new Set(pendingCapIds);
			const alreadyTracked = new Set([...activeCapIds, ...pendingCapIds]);
			const newOnly = capabilityIds.filter((c) => !alreadyTracked.has(c));

			if (newOnly.length === 0) {
				const allActive = capabilityIds.every((c) => alreadyActive.has(c));
				if (allActive) {
					throw agentError("CONFLICT", ERR.ALREADY_GRANTED);
				}

				// Some still pending — return pending with approval info
				const stillPending = capabilityIds.filter((c) => alreadyPending.has(c));
					...
					return ctx.json({
						...
						approval: {
							...
							expires_in: Math.floor(
								(new Date(existingApproval.expiresAt).getTime() -
									now.getTime()) /
									1000
							),
			...
			if (
				...
				agentSession.agent.mode === "autonomous" &&
				!hostUserId
			) {
				needsApproval = [];
			}
