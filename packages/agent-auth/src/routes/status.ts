...
export function agentStatus(opts: ResolvedAgentAuthOptions) {
  return createAuthEndpoint(
    "/agent/status",
    {
      ...
      metadata: {
        openapi: {
          description: "Returns the current status of an agent and its capability grants (§6.5).",
        },
      },
    },
    async (ctx) => {
      ...
      if (agent.status === "active" && agent.mode === "autonomous") {
        const pendingClaim = await ctx.context.adapter.findMany<ApprovalRequest>({
