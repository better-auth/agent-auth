...
export function approveCapability(
  ...
  return createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      if (approvalId) {
        approvalRequest = await ctx.context.adapter.findOne<ApprovalRequest>({
      ...
      const isClaim = agent.mode === "autonomous" && agent.status === "active";

      if (!isClaim && agent.userId && agent.userId !== session.user.id) {
      ...
      if (isClaim) {
        ...
        if (action === "deny") {
          ...
          void deliverApprovalNotifications(resolved, {
            ...
            status: "denied",
            error: "access_denied",
            message: denyReason ?? "User denied the claim request.",
          });
          emit(
            opts,
            {
          ...
          return ctx.json({ status: "denied", agentId });
        }
        ...
        if (targetHost && !targetHost.userId) {
          await ctx.context.adapter.update({
            model: TABLE.host,
            where: [{ field: "id", value: targetHost.id }],
            update: {
              userId: session.user.id,
              updatedAt: now,
            },
        ...
        emit(
          opts,
          {
            ...
            agentId,
            hostId: agent.hostId,
            metadata: { capabilities: activeCaps },
          },
          ctx,
        );
      ...
      const pendingGrants = allGrants.filter((g) => g.status === "pending");
      ...
      if (action === "deny") {
        ...
        emit(
          opts,
          {
            ...
            agentId,
            metadata: {
              capabilities: pendingGrants.map((g) => g.capability),
      ...
      if (opts.proofOfPresence.enabled && challengeCache) {
        ...
        if (requiresWebAuthn) {
          ...
          try {
            passkeys = await ctx.context.adapter.findMany<StoredPasskey>({
              model: "passkey",
          ...
          if (!expectedChallenge) {
            throw agentError(
              "FORBIDDEN",
              ERR.WEBAUTHN_VERIFICATION_FAILED,
          ...
          try {
            ...
            await ctx.context.adapter.update({
              model: "passkey",
              where: [{ field: "id", value: matchingPasskey.id }],
              update: {
      ...
      for (const g of allGrants) {
        ...
        list.push(g);
        activeGrantsByCapability.set(g.capability, list);
      }
      const added: string[] = [];
      ...
      for (const grant of pendingGrants) {
        if (approvedCapIds.has(grant.capability)) {
          ...
          if (alreadyCovered) {
            ...
            const list = activeGrantsByCapability.get(grant.capability) ?? [];
            list.push(grant as AgentCapabilityGrant);
            activeGrantsByCapability.set(grant.capability, list);
      ...
      await activatePendingAgent(ctx.context.adapter, opts, ctx, {
        agentId,
        userId: session.user.id,
        agent,
      });
