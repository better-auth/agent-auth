...
export function executeCapability(opts: ResolvedAgentAuthOptions) {
  return createAuthEndpoint(
    "/capability/execute",
    {
      ...
      metadata: {
        openapi: {
          description: "Execute a granted capability on behalf of the agent (§6.11).",
        },
      },
    },
    async (ctx) => {
      ...
      let constrainedGrants: AgentCapabilityGrant[] = [];
      ...
      if (sessionGrant) {
        ...
        if (!activeGrant) {
          constrainedGrants = activeGrants.filter((g) => g.constraints);
        }
      }

      if (!activeGrant && constrainedGrants.length > 0) {
        // Agent has grants but constraints don't allow these args —
        // validate against the first constrained grant to produce
        // a proper constraint_violated / unknown_constraint_operator error.
        const grant = constrainedGrants[0];
        const constraintArgs = (args ?? {}) as Record<string, ConstraintPrimitive | undefined>;
        const result = validateConstraints(grant.constraints!, constraintArgs);
        if (result.unknownOperators.length > 0) {
          throw agentError("BAD_REQUEST", ERR.UNKNOWN_CONSTRAINT_OPERATOR, undefined, undefined, {
            operators: result.unknownOperators,
          });
        }
        if (result.violations.length > 0) {
          throw agentError("FORBIDDEN", ERR.CONSTRAINT_VIOLATED, undefined, undefined, {
      ...
      if (!activeGrant) {
        activeGrant = await tryAutoGrantFromHostBudget(ctx.context.adapter, opts, ctx, {
      ...
      try {
        ...
        throw agentError("INTERNAL_SERVER_ERROR", ERR.INTERNAL_ERROR, execError);
      }
