...
import { displayName, formatGrantsResponse } from "./_helpers";
  ...
  return createAuthEndpoint(
    "/agent/list",
    {
      ...
      query: z
        .object({
          status: z
            .enum(["active", "pending", "expired", "revoked", "rejected", "claimed"])
            .optional(),
      ...
      metadata: {
        openapi: {
          description: "List agents for the current user with filtering and pagination (§8).",
        },
      },
    },
    async (ctx) => {
      ...
      await Promise.all(
        hostIds.map(async (hid) => {
          ...
          hostMap.set(hid, displayName(host?.name, hid, "Device"));
        }),
      );
      ...
      const agentsWithGrants = await Promise.all(
        agents.map(async (agent) => {
          ...
          return {
            agent_id: agent.id,
            name: displayName(agent.name, agent.id, "Agent"),
            status: agent.status,
            mode: agent.mode,
            host_id: agent.hostId,
            host_name: hostMap.get(agent.hostId) ?? displayName(null, agent.hostId, "Device"),
