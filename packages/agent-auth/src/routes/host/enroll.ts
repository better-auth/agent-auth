    ...
    "/host/enroll",
    {
      ...
      body: z.object({
        ...
        public_key: z
          ...
          .meta({
            description: "Host Ed25519 public key as JWK, generated on the device.",
          }),
      ...
      metadata: {
        openapi: {
          description: "Enroll a device as a host using a one-time enrollment token (§3.2).",
        },
      },
    },
    async (ctx) => {
      ...
      if (host.enrollmentTokenExpiresAt && new Date(host.enrollmentTokenExpiresAt) <= new Date()) {
      ...
      if (existing && existing.id !== host.id) {
        ...
        return ctx.json({
          hostId: existing.id,
          name: name ?? host.name ?? existing.name,
          default_capabilities: parseCapabilityIds(existing.defaultCapabilities),
          status: "active",
        });
      }
