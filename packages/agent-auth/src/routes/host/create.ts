  ...
  return createAuthEndpoint(
    "/host/create",
    {
      ...
      body: z.object({
        ...
        jwks_url: z.string().url().optional().meta({
          description: "JWKS URL for remote key discovery. If provided, public_key is optional.",
        }),
        default_capabilities: z.array(z.string()).optional().meta({
          description: "Default capabilities agents inherit. Reactivated agents reset to these.",
        }),
      }),
    ...
    async (ctx) => {
      ...
      if (isEnrollmentFlow) {
        ...
        enrollmentTokenExpiresAt = new Date(now.getTime() + DEFAULTS.enrollmentTokenTTL * 1000);
      }

      const host = await ctx.context.adapter.create<Record<string, unknown>, AgentHost>({
        model: TABLE.host,
        data: {
          name: hostName ?? null,
          userId: session.user.id,
