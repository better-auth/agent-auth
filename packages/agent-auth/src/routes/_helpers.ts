 ...
 */
export function resolveDeviceAuthPage(opts: ResolvedAgentAuthOptions, origin: string): string {
...
export function activeGrants(grants: AgentCapabilityGrant[]): AgentCapabilityGrant[] {
  const now = new Date();
...
export async function buildApprovalInfo(
  ...
  const method = opts.approvalMethods.includes(resolved) ? resolved : "device_authorization";
    ...
    if (user) {
      await adapter.create<Record<string, unknown>, ApprovalRequest>({
        ...
        data: {
          ...
          bindingMessage:
            context.bindingMessage ?? `Agent "${context.agentName}" requesting approval`,
...
export function verifyAudience(
  ...
  const accepted = new Set([configuredOrigin, base, `${base}/capability/execute`]);
...
export async function claimAutonomousAgents(
  ...
  const now = new Date();
  const autonomous = agents.filter((a) => a.mode === "autonomous" && a.status === "active");
  ...
  for (const agent of autonomous) {
    ...
    const activeCapabilities = grants.filter((g) => g.status === "active").map((g) => g.capability);
...
export async function tryAutoGrantFromHostBudget(
  ...
  const now = new Date();
  const grant = await adapter.create<Record<string, unknown>, AgentCapabilityGrant>({
    model: TABLE.grant,
    data: {
      agentId: params.agentId,
...
export async function deliverApprovalNotifications(
  ...
  for (const req of requests) {
    if (req.method !== "ciba" || !req.clientNotificationEndpoint || !req.clientNotificationToken)
      continue;
    if (req.deliveryMode !== "ping" && req.deliveryMode !== "push") continue;
    try {
...
/**
 * Generate a short display name when a stored name is null.
 * Uses the first 8 chars of the ID to create a readable fallback
 * like "Device a1b2c3d4" or "Agent a1b2c3d4".
 */
export function displayName(name: string | null | undefined, id: string, prefix: string): string {
  if (name) return name;
