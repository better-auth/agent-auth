...
export function listCapabilities(opts: ResolvedAgentAuthOptions) {
  return createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const cursorIdx = ctx.query?.cursor ? Number.parseInt(ctx.query.cursor, 10) : 0;
      ...
      if (agentSession) {
        ...
        grantedCapabilityIds = new Set(
          grants
            .filter(
              (g) => g.status === "active" && (!g.expiresAt || new Date(g.expiresAt) > new Date()),
            )
            .map((g) => g.capability),
        );
      } else if (hostSession) {
        grantedCapabilityIds = new Set(hostSession.host.defaultCapabilities ?? []);
      }
      ...
      return ctx.json({
        capabilities: page.map((c) => {
          ...
          if (c.approvalStrength) {
            entry.approval_strength = c.approvalStrength;
          }
          if (grantedCapabilityIds) {
            entry.grant_status = grantedCapabilityIds.has(c.name) ? "granted" : "not_granted";
          }
