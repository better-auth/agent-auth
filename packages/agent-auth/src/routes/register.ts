...
export function register(
  ...
  return createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const bodyHostName = rawHostName
        ? sanitizeDisplayText(rawHostName, DISPLAY_LIMITS.hostName)
        : undefined;
      ...
      if (decoded.agent_public_key && typeof decoded.agent_public_key === "object") {
        agentPublicKey = decoded.agent_public_key as Record<string, unknown>;
      }
      if (decoded.agent_jwks_url && typeof decoded.agent_jwks_url === "string") {
      ...
      if (hostRecord) {
        ...
        if (mode === "autonomous" && !hostRecord.userId && hostRecord.status === "pending") {
        ...
        if (opts.agentSessionTTL > 0) {
          heartbeat.expiresAt = new Date(Date.now() + opts.agentSessionTTL * 1000);
        }
        ctx.context.runInBackground(
          ctx.context.adapter
            .update({
        ...
        if (existingHost) {
          ...
          hostDefaultCaps = parseCapabilityIds(existingHost.defaultCapabilities);
          ...
          if (mode === "autonomous" && !existingHost.userId && existingHost.status === "pending") {
            bgUpdates.status = "active";
            bgUpdates.activatedAt = new Date();
            hostRecord = {
              ...hostRecord,
          ...
          const newHost = await ctx.context.adapter.create<Record<string, unknown>, AgentHost>({
            model: TABLE.host,
            data: {
      ...
      const delegatedWithoutUser = mode === "delegated" && !userId;
      ...
      if (hostDefaultCaps !== null && hostDefaultCaps.length > 0) {
        ...
        if (requestedCapIds && requestedCapIds.length > 0) {
          resolvedCapNames = requestedCapIds.filter((c) => hasCapability(budget, c));
          pendingCapNames = requestedCapIds.filter((c) => !hasCapability(budget, c));
        } else {
      ...
      // ---------- force_approval / delegated-without-user: move all resolved caps to pending ----------
      if ((forceApproval || delegatedWithoutUser) && resolvedCapNames.length > 0) {
        pendingCapNames = [...resolvedCapNames, ...pendingCapNames];
        resolvedCapNames = [];
      }

      const agentUserId = forceApproval || delegatedWithoutUser ? null : userId;
      ...
      const needsApproval =
        forceApproval || isHostPending || pendingCapNames.length > 0 || delegatedWithoutUser;
      ...
      if (pendingCapNames.length > 0) {
        await createGrantRows(ctx.context.adapter, agent.id, pendingCapNames, agentUserId, {
          status: "pending",
          reason:
            reason ??
            (forceApproval
