...
import { buildApprovalInfo, createGrantRows, formatGrantsResponse } from "./_helpers";
...
export function reactivateAgent(opts: ResolvedAgentAuthOptions) {
  return createAuthEndpoint(
    "/agent/reactivate",
    {
      ...
      metadata: {
        openapi: {
          description: "Reactivate an expired agent (§6.6). Capabilities decay to host defaults.",
        },
      },
    },
    async (ctx) => {
      ...
      if (agent.status === "active") {
        ...
        return ctx.json({
          ...
          created_at: agent.createdAt ? new Date(agent.createdAt).toISOString() : null,
          last_used_at: agent.lastUsedAt ? new Date(agent.lastUsedAt).toISOString() : null,
      ...
      if (opts.absoluteLifetime > 0 && agent.createdAt) {
        const absoluteExpiry = new Date(agent.createdAt).getTime() + opts.absoluteLifetime * 1000;
