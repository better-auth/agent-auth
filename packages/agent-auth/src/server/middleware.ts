	...
	try {
		...
		if (typ === "host+jwt") {
			...
			const hostAllowed =
				...
				(host.status === "active" ||
					(host.status === "pending" && rctx.path === "/agent/status"));
		...
		await jtiCache.add(
			jtiKey,
			opts.jwtMaxAge + CLOCK_SKEW_TOLERANCE_SEC,
		);
		...
		if (payload.htm || payload.htu || payload.ath) {
			if (payload.htm) {
				...
				if (
					String(payload.htm).toUpperCase() !==
					method.toUpperCase()
				) {
					throw agentError(
						"UNAUTHORIZED",
			...
			if (payload.ath && rctx.body) {
				const bodyStr =
					typeof rctx.body === "string"
						? rctx.body
						: JSON.stringify(rctx.body);
		...
		const agentSession: AgentSession = {
			...
			host: host
				? { id: host.id, userId: host.userId, status: host.status }
				: null,
			user,
		};
