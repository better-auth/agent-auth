...
export function resolveDeviceAuthPage(opts: ResolvedServerOptions, origin: string): string {
  ...
  for (const cap of capabilityIds) {
    const expiresAt =
      status === "active" && ttlContext
        ? await resolveGrantExpiresAt(ttlContext.pluginOpts as any, cap, {
            agentId,
...
export function validateCapabilityIds(capabilityIds: string[], opts: ResolvedServerOptions): void {
...
export function emitEvent(opts: Pick<ResolvedServerOptions, "onEvent">, event: any): void {
