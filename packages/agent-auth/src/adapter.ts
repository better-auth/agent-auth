...
export function getAgentAuthAdapter(adapter: FullAdapter, opts: ResolvedAgentAuthOptions) {
  ...
  const findAgentsByUser = (
    ...
    adapter.findMany<Agent>({
      ...
      where: [
        { field: "userId", value: userId },
        ...(filters?.status ? [{ field: "status", value: filters.status }] : []),
        ...(filters?.hostId ? [{ field: "hostId", value: filters.hostId }] : []),
      ],
   ...
   */
  const transparentReactivation = async (agent: Agent): Promise<Agent | null> => {
