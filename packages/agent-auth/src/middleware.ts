...
export function createAgentAuthBeforeHook(
  ...
  return {
    ...
    handler: createAuthMiddleware(async (ctx) => {
      ...
      try {
        const db = getAgentAuthAdapter(ctx.context.adapter as FullAdapter, opts);
        ...
        if (typ === "host+jwt") {
          ...
          if (!hostPubKey && host.publicKey) {
            try {
              ...
              throw agentError("UNAUTHORIZED", AGENT_AUTH_ERROR_CODES.INVALID_PUBLIC_KEY);
            }
          }
          if (!hostPubKey) {
            throw agentError("UNAUTHORIZED", AGENT_AUTH_ERROR_CODES.INVALID_PUBLIC_KEY);
        ...
        if (opts.absoluteLifetime > 0 && agent.createdAt) {
          ...
          if (Date.now() >= absExpiry) {
            ...
            throw agentError("FORBIDDEN", AGENT_AUTH_ERROR_CODES.ABSOLUTE_LIFETIME_EXCEEDED);
          }
        }
        ...
        if (payload.htm || payload.htu || payload.ath) {
          if (payload.htm) {
            ...
            if (String(payload.htm).toUpperCase() !== method.toUpperCase()) {
              throw agentError("UNAUTHORIZED", AGENT_AUTH_ERROR_CODES.REQUEST_BINDING_MISMATCH);
            }
          }
          if (payload.htu) {
        ...
        if (needsReactivation) {
          ...
          if (!reactivated) {
            throw agentError("UNAUTHORIZED", AGENT_AUTH_ERROR_CODES.AGENT_EXPIRED);
          }
          agent = reactivated;
          emit(
            opts,
