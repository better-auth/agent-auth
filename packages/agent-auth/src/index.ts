    ...
    init(ctx) {
      ...
      if (ctx.secondaryStorage) {
        const jtiUseSecondary =
          opts.jtiCacheStorage === "secondary-storage" || opts.jtiCacheStorage !== "memory";
        if (jtiUseSecondary) {
          jtiCache.useSecondaryStorage(ctx.secondaryStorage);
        }

        const jwksUseSecondary =
          opts.jwksCacheStorage === "secondary-storage" || opts.jwksCacheStorage !== "memory";
        if (jwksUseSecondary) {
...
export { agentError, AGENT_AUTH_ERROR_CODES } from "./errors";
