...
describe("Host Management", () => {
  ...
  it("creates a host via session auth (POST /host/create)", async () => {
    ...
    const body = await json<{ hostId: string; status: string; default_capabilities: string[] }>(
      res,
    );
...
describe("Capabilities Endpoint — location field (§2.15)", () => {
  ...
  it("includes location in listing when capability has one", async () => {
    ...
    const res = await t.auth.handler(new Request(`${API}/capability/list`, { method: "GET" }));
    expect(res.ok).toBe(true);
    const body = await json<{
...
describe("Constraints (§2.13)", () => {
  ...
  it("registers agent with constrained capabilities", async () => {
    ...
    const res = await client.api("/agent/register", {
      ...
      body: JSON.stringify({
        ...
        capabilities: [
          ...
          {
            name: "transfer",
            constraints: { amount: { max: 1000 }, currency: { in: ["USD", "EUR"] } },
          },
        ],
        mode: "delegated",
      }),
    ...
    expect((transferGrant!.constraints as Record<string, Record<string, number>>).amount.max).toBe(
      1000,
    );
  });
  ...
  it("request-capability with constraints stores them on grant", async () => {
    ...
    const res = await client.api("/agent/request-capability", {
      ...
      headers: { authorization: `Bearer ${jwt}` },
      body: JSON.stringify({
        capabilities: [{ name: "transfer", constraints: { amount: { max: 500 } } }],
      }),
    });
    ...
    const regRes = await t.auth.handler(
      new Request(`${API}/agent/register`, {
        ...
        body: JSON.stringify({
          ...
          capabilities: [{ name: "transfer", constraints: { amount: { max: 100 } } }],
          mode: "delegated",
        }),
      }),
    );
    ...
    const regRes = await t.auth.handler(
      new Request(`${API}/agent/register`, {
        ...
        body: JSON.stringify({
          ...
          capabilities: [{ name: "transfer", constraints: { amount: { bogus_op: 999 } } }],
          mode: "delegated",
        }),
      }),
    );
