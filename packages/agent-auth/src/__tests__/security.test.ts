...
describe("Transparent Reactivation", () => {
  it("emits agent.reactivated with actorType 'system' and transparent flag", async () => {
    ...
    const reactivationEvent = events.find(
      (e) => e.type === "agent.reactivated" && "actorType" in e && e.actorType === "system",
    );
...
describe("verifyAudience — location model", () => {
  ...
  it("accepts the default execute endpoint", () => {
    expect(verifyAudience("http://localhost:3000/api/auth/capability/execute", baseURL)).toBe(true);
  });

  it("rejects an unrelated URL without expectedLocation", () => {
    expect(verifyAudience("https://external.example.com/execute", baseURL)).toBe(false);
  });
  ...
  it("rejects aud that doesn't match expectedLocation", () => {
    ...
    expect(verifyAudience(wrongAud, baseURL, null, false, location)).toBe(false);
  });
  ...
  it("does NOT accept capability B's location when expectedLocation is capability A's", () => {
    ...
    expect(verifyAudience(locationB, baseURL, null, false, locationA)).toBe(false);
  });

  it("handles array aud — accepts if any value matches", () => {
    const location = "https://external.example.com/execute";
    expect(
      verifyAudience(["https://wrong.example.com", location], baseURL, null, false, location),
    ).toBe(true);
  });
  ...
  it("root baseURL produces correct execute endpoint", () => {
    expect(
      verifyAudience("http://localhost:3000/capability/execute", "http://localhost:3000"),
    ).toBe(true);
  });
  ...
  it("accepts proxy full base URL as audience", () => {
    ...
    expect(verifyAudience("https://proxy.example.com/api/auth", baseURL, headers, true)).toBe(true);
  });
});
...
describe("Startup URL validation for capability locations", () => {
  it("throws on invalid location URL", () => {
    expect(() =>
      _agentAuth({
        capabilities: [{ name: "bad", description: "bad cap", location: "not-a-url" }],
      }),
  ...
  it("throws with capability name in error message", () => {
    expect(() =>
      _agentAuth({
        capabilities: [{ name: "my_cap", description: "test", location: "foo" }],
      }),
    ).toThrow(/my_cap/);
  });
...
describe("Grant Revocation Consistency", () => {
  it("agent revoke → all grants become revoked", async () => {
    ...
    expect(statusRes.ok).toBe(true);
    const statusBody = await json<{ agent_capability_grants: Array<{ status: string }> }>(
      statusRes,
    );
