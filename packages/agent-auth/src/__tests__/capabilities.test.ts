...
describe("mergeCapabilities", () => {
  ...
  it("wildcard subsumes specific IDs", () => {
    const result = mergeCapabilities(["github.*"], ["github.create_issue", "github.list_issues"]);
...
describe("findBlockedCapabilities", () => {
  ...
  it("finds blocked capabilities", () => {
    expect(findBlockedCapabilities(["read", "admin"], ["admin"])).toEqual(["admin"]);
  });

  it("wildcard block catches specific IDs", () => {
    expect(findBlockedCapabilities(["github.create_issue", "slack.send"], ["github.*"])).toEqual([
      "github.create_issue",
    ]);
  });
});
