...
describe("validateConstraints", () => {
	...
	it("validates in operator", () => {
		expect(validateConstraints({ currency: { in: ["USD", "EUR"] } }, { currency: "USD" }).valid).toBe(true);
		expect(validateConstraints({ currency: { in: ["USD", "EUR"] } }, { currency: "GBP" }).valid).toBe(false);
	});

	it("validates not_in operator", () => {
		expect(validateConstraints({ currency: { not_in: ["GBP"] } }, { currency: "USD" }).valid).toBe(true);
		expect(validateConstraints({ currency: { not_in: ["GBP"] } }, { currency: "GBP" }).valid).toBe(false);
	});
