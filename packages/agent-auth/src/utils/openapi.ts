...
function buildInputSchema(
  ...
  if (requestBody) {
    ...
    if (jsonContent?.schema) {
      const bodySchema = deref(spec, jsonContent.schema) as Record<string, unknown>;
...
function buildOperationMap(spec: OpenAPISpec, baseUrl: string): Map<string, ResolvedOperation> {
...
export function createOpenAPIHandler(
  ...
  return async ({ ctx, capability, arguments: args, agentSession }) => {
    ...
    if (!op) {
      throw new Error(`No OpenAPI operation found for capability "${capability}".`);
    }
    ...
    if (response.status === 202) {
      ...
      const statusUrl =
        (body.status_url as string | undefined) ?? response.headers.get("location") ?? "";
      const retryAfter = response.headers.get("retry-after");
      return asyncResult(statusUrl, retryAfter ? parseInt(retryAfter, 10) : undefined);
    }
...
function resolveDefaultHostCaps(
  ...
  if (typeof filter === "function") {
    const capInfos: Array<OpenAPICapabilityInfo & { _name: string }> = parsed.map((c) => ({
      name: c.name,
  ...
  const methods = new Set((Array.isArray(filter) ? filter : [filter]).map((m) => m.toUpperCase()));
...
export function createFromOpenAPI(
  ...
  AgentAuthOptions,
  "providerName" | "providerDescription" | "capabilities" | "onExecute" | "defaultHostCapabilities"
> {
  const parsed = parseOpenAPICapabilities(spec);
  const capabilities: Capability[] = parsed.map(({ _method, ...cap }) => {
    const strength = resolveApprovalStrength({ ...cap, _method }, opts.approvalStrength);
  ...
  const defaultHostCapabilities = resolveDefaultHostCaps(parsed, opts.defaultHostCapabilities);

  return {
    ...(spec.info?.title ? { providerName: spec.info.title } : {}),
    ...(spec.info?.description ? { providerDescription: spec.info.description } : {}),
    capabilities,
    onExecute,
    ...(defaultHostCapabilities !== undefined ? { defaultHostCapabilities } : {}),
  };
}
