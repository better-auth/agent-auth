...
export function hasAllCapabilities(granted: string[], required: string[]): boolean {
  return required.every((r) => hasCapability(granted, r));
}

/** Check if all `capabilityIds` are covered by `allowed`. */
export function isSubsetOf(capabilityIds: string[], allowed: string[]): boolean {
...
export function findBlockedCapabilities(capabilityIds: string[], blocked: string[]): string[] {
  if (blocked.length === 0) return [];
  return capabilityIds.filter((s) => blocked.some((b) => capabilityCovers(b, s)));
}
