...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-medium ring-1 ${c.bg} ${c.text} ${c.ring}`}
    >
...
function EventTypeBadge({ type }: { type: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ${c.bg} ${c.text} ${c.ring}`}
    >
      <span className={`h-1.5 w-1.5 rounded-full ${c.dot}`} />
      {type}
    </span>
      ...
      {logs.map((log, i) => {
        ...
        return (
          ...
          >
            ...
            <div
              className={`flex-1 ml-1 mb-1.5 rounded-lg border border-transparent transition-colors ${isExpanded ? "bg-white/3 border-border/50" : "group-hover:bg-white/2"}`}
            >
...
function MetaItem({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1 rounded-lg bg-white/3 px-3.5 py-2.5 ring-1 ring-white/4">
      <span className="text-[10px] uppercase tracking-wider text-muted/70 font-medium">
        {label}
      </span>
  ...
  return (
    <div className="rounded-lg bg-white/3 px-3.5 py-2.5 ring-1 ring-white/4 group transition-colors hover:bg-white/5">
      <div className="flex items-center gap-3">
        ...
        <code className="flex-1 text-[12px] font-mono text-foreground/80 truncate">
          {grant.capability}
        </code>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-2 text-[11px] text-muted/70">
                        <span>
                          {activeGrants.length} capability{activeGrants.length !== 1 ? "ies" : "y"}
                        </span>
                          ...
                          <div className="flex flex-col gap-4">
                            ...
                            <div>
                              ...
                              {editingAgent === agent.agent_id ? (
                                    ...
                                    {availableCaps.map((cap) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          />
                                          <div className="flex-1 min-w-0">
                                            <code className="text-[12px] font-mono text-foreground/80 truncate block">
                                              {cap.name}
                                            </code>
                                            {cap.description && (
                                ...
                                <p className="text-xs text-muted/50 py-2">
