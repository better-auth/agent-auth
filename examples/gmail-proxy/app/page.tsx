...
function GmailLogo({ className }: { className?: string }) {
	return (
		<svg className={className} viewBox="0 0 75 75" xmlns="http://www.w3.org/2000/svg">
			...
			<path d="M68.75 18.75v0L62.5 25l-25 18.75L18.75 28.125V62.5h-6.25c-3.45 0-6.25-2.8-6.25-6.25v-37.5" fill="#34A853" fillOpacity="0"/>
	...
	return (
		<div className="flex min-h-screen flex-col items-center justify-center bg-white">
			<main className="flex w-full max-w-[420px] flex-col items-center px-8 py-12">
				...
				<h1 className="mt-6 text-[24px] font-normal text-foreground">
					Gmail Proxy
				</h1>

				<p className="mt-2 text-center text-sm leading-relaxed text-muted">
					Proxy Gmail API access for AI agents through the Agent Auth Protocol. Sign in to connect your account.
				</p>
				...
				<div className="mt-8 w-full">
					<button
						...
						disabled={signingIn}
						className="flex h-10 w-full cursor-pointer items-center justify-center gap-3 rounded-full border border-border bg-white text-sm font-medium text-foreground shadow-sm transition-shadow hover:shadow-md active:bg-surface disabled:pointer-events-none disabled:opacity-60"
					>
						{signingIn ? (
							<Spinner />
						) : (
				...
				<p className="mt-6 text-center text-xs leading-relaxed text-muted">
					Grants read, send, and management access to your Gmail messages, threads, labels, and drafts.
				</p>

				<div className="mt-10 flex items-center gap-5 text-[11px] font-medium uppercase tracking-wider text-muted">
					<span>OAuth 2.0</span>
