...
function StatusBadge({ status }: { status: string }) {
	const styles: Record<string, string> = {
		...
		pending_enrollment: "bg-gmail-yellow/15 text-gmail-yellow ring-1 ring-gmail-yellow/30",
	...
	return (
		<span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-medium ${styles[status] ?? "bg-gray-100 text-gray-500 ring-1 ring-gray-200"}`}>
			{status.replace("_", " ")}
		</span>
	);
}
		...
		<div className="mx-auto w-full max-w-3xl px-6 py-8">
			<div className="flex flex-col gap-6">
				<div className="flex items-start justify-between">
					<div>
						<h1 className="text-[22px] font-normal text-foreground">Hosts</h1>
				...
				{loading ? (
					...
					<div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border py-16">
						...
						<p className="mt-1 text-xs text-muted">Hosts are created when agents register from new environments.</p>
					</div>
				) : (
					<div className="flex flex-col gap-2.5">
						{hosts.map((host) => {
							...
							return (
								<div key={host.id} className="rounded-2xl border border-border bg-white shadow-sm">
									<button
										...
										<div className="border-t border-border px-5 py-4">
											<div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-4">
												<div>
													...
													<code className="text-xs font-mono text-foreground break-all">{host.id}</code>
												</div>
												<div>
													<p className="text-[10px] uppercase tracking-widest text-muted">Name</p>
													<p className="text-xs text-foreground">{host.name ?? "—"}</p>
												</div>
												<div>
													<p className="text-[10px] uppercase tracking-widest text-muted">Last Used</p>
													<p className="text-xs text-foreground">{timeAgo(host.last_used_at)}</p>
												</div>
												<div>
													<p className="text-[10px] uppercase tracking-widest text-muted">Activated</p>
												...
												<div>
													<p className="text-[10px] uppercase tracking-widest text-muted">Updated</p>
													<p className="text-xs text-foreground">{timeAgo(host.updated_at)}</p>
												</div>
											</div>

											<div className="mb-4">
												<div className="flex items-center justify-between mb-2">
													<p className="text-[10px] uppercase tracking-widest text-muted">Default Capabilities</p>
													{editingHost === host.id ? (
												...
												{editingHost === host.id ? (
													...
													<p className="text-xs text-muted py-2">No default capabilities set.</p>
												)}
											</div>
											...
											{host.status === "active" && (
												<button
													...
													className="cursor-pointer rounded-full border border-gmail-red/20 px-4 py-1.5 text-xs font-medium text-gmail-red transition-colors hover:bg-gmail-red/5 disabled:opacity-50"
												>
